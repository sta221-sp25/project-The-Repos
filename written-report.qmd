...
As we can see in a quick summary of our data, the variable we are most interested in – the final score of users – ranges from 0 to 10, which makes sense as 10 questions can be answered either 'Yes' or 'No'. Generally, the scores are a bit right-skewed, with a single peak around 4 or 5 points. The mean score is 5.077, and the median is 5, both of which are relatively high considering that scores above 6 warrant further diagnostic evaluation. However, because suspecting a diagnosis is a reason for taking the test, these statistics are reasonable reflections of the test-taking population. We then observe that roughly 30% of test takers are encouraged to seek a diagnosis due to a score higher than 6. We also observe the IQR to be 4 points, as most test takers have a score between 3 and 7 points inclusive. In the context of the data, such a spread is reasonable, and no outliers exist.
...
autism_ds |>
  ...
  labs(x = "Test Result",
       y = "Count",
       title = "Distribution of Autism Screening Results") +
  theme_minimal()
```

Furthermore, the chart above provides a visual representation of test takers advised to pursue further professional evaluation for ASD. A notable proportion of test takers were informed they had traits or behaviors indicative of autism, though the count is still significantly less than those not encouraged.
...
```{r}
### Q5, Q7 predictive relationship
q5_model <- glm(i_can_tell_if_someone_bored ~ 1, 
              data = autism_ds, family = "binomial")
q57_model <- glm(i_can_tell_if_someone_bored ~ i_can_tell_feelings_from_faces, 
              data = autism_ds, family = "binomial")

cat("D-reduced:",(deviance_q5 <- -2 * glance(q5_model)$logLik), "\n")
cat("D-full:",(deviance_q57 <- -2 * glance(q57_model)$logLik), "\n")
cat("G-stat", (G <- deviance_q5 - deviance_q57), "\n")
cat("p-value:",pchisq(G, df = 1, lower.tail = FALSE), "\n")

tidy(q57_model) |>
  kable(digits = 3)

### Q5, Q10 predictive relationship
q510_model <- glm(i_can_tell_if_someone_bored ~ i_find_it_hard_to_figure_out_others_intentions, 
              data = autism_ds, family = "binomial")

cat("D-reduced:",(deviance_reduced <- -2 * glance(reduced_model)$logLik), "\n")
cat("D-full:",(deviance_full <- -2 * glance(q510_model)$logLik), "\n")
cat("G-stat", (G <- deviance_reduced - deviance_full), "\n")
cat("p-value:",pchisq(G, df = 1, lower.tail = FALSE), "\n")
...
### Q7, Q10 predictive relationship
autism_ds <- autism_ds %>%
  mutate(highProb = if_else(result > 6, 1, 0))

reduced_model <- glm(i_can_tell_feelings_from_faces ~ 1, 
              data = autism_ds, family = "binomial")
q710_model <- glm(i_can_tell_feelings_from_faces ~ i_find_it_hard_to_figure_out_others_intentions, 
              data = autism_ds, family = "binomial")

cat("D-reduced:",(deviance_reduced <- -2 * glance(reduced_model)$logLik), "\n")
cat("D-full:",(deviance_full <- -2 * glance(q710_model)$logLik), "\n")
cat("G-stat", (G <- deviance_reduced - deviance_full), "\n")
cat("p-value:",pchisq(G, df = 1, lower.tail = FALSE), "\n")
...
```{r}
### Q6, Q8 predictive relationship
q6_model <- glm(i_can_go_back_to_work_when_interrupted ~ 1, 
              data = autism_ds, family = "binomial")
q68_model <- glm(i_can_go_back_to_work_when_interrupted ~ i_can_multi_task, 
              data = autism_ds, family = "binomial")

cat("D-reduced:",(deviance_q6 <- -2 * glance(q6_model)$logLik), "\n")
cat("D-full:",(deviance_q68 <- -2 * glance(q68_model)$logLik), "\n")
cat("G-stat", (G <- deviance_q6 - deviance_q68), "\n")
cat("p-value:",pchisq(G, df = 1, lower.tail = FALSE), "\n")
...
Then to consider adding the behavioral question Q8 or not, we first fit as following

$$log(\frac{\hat\pi}{1-\hat\pi})=\beta_0 + \beta_{ethnicity}X_{ethnicity}+\beta_{gender}X_{gender}+\beta_{jaundice}X_{jaundice}+\beta_{Q8}X_{Q8}$$
...
To compare the two models, where model one does not include the variable for Q8 and model two does, both AIC and BIC of model two are smaller than the values of model one. Therefore, we choose model two.
...
```{r}
autism_ds_aug <- autism_ds
autism_ds_aug$predicted_prob <- predict(final_lasso_model, newx = X, type = "response")
```

```{r, echo = FALSE}
# Ensure predicted_prob is a numeric vector for the positive class (1)
predicted_prob_numeric <- autism_ds_aug$predicted_prob[, 1]  # Assuming column 2 is the probability for "case" (1)

# Create ROC curve again with corrected predicted probabilities
roc_curve <- roc(autism_ds_aug$highProb, predicted_prob_numeric)
...
The area under the curve is 0.795, which for a model fitted to demographic data and only one question on the quiz, indicates that further investigation into how these questions may over- or under-diagnose certain populations would be worth pursuing.

## Results

```{r}
final_model_fit <- glm(highProb ~ 
                       ethnicity + gender + jundice + 
                       i_can_go_back_to_work_when_interrupted,
                       data = autism_ds, 
                       family = binomial)
...
final_model_fit_aug <- augment(final_model_fit)

pred_prob <- predict.glm(final_model_fit, type = "response")

final_model_fit_aug <- final_model_fit_aug |>
  mutate(
    highProb = factor(highProb, levels = c(0, 1))  # 1 = "event", 0 = "non-event"
  )
...
As previously discussed, the logistic regression model for determining the probability of a high score on the ASD questionnaire possesses a 0.795 AUC with predictors of demographic data and specifically question #8. This is a fairly high AUC given the context of primarily relying on demographic data rather than actual behaviors For model assumptions, logistic regression requires the assumption of log-odds possessing linearity, randomness, and independence of observations. It is worth noting that of these conditions, randomness is not necessarily satisfied due to the nature of the data set – it was collected by people who already were likely to suspect an ASD diagnosis, as they are those who took the test voluntarily. However, our other two assumptions are likely to hold for this model, and we still determine it worth further investigation.

We would check linearity as below, noting that independence is likely to be satisfied and does not require further investigation, but as none of our predictors are quantitative and our all categorical, we instead do not need to.

Once more, a .795 AUC for factors primarily independent of autistic behaviors indicate that further research is required in the diagnostic criteria and formulation of questionnaires of for autism. It is worth ensuring that everyone can receive the appropriate diagnosis, and thus, plan for accommodations for improvement of quality of life.
