...
cat("D-reduced:",(deviance_reduced <- -2 * glance(reduced_model)$logLik), "\n")
cat("D-full:",(deviance_full <- -2 * glance(q1_model)$logLik), "\n")
cat("G-stat", (G <- deviance_reduced - deviance_full), "\n")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ small_sounds, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ difficult_to_understand_char, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ ease_to_read_between_lines, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ focus_on_whole_picture, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_can_tell_if_someone_bored, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_can_multi_task, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_can_tell_feelings_from_faces, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_can_go_back_to_work_when_interrupted, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_like_to_collect_info_on_categories, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ i_find_it_hard_to_figure_out_others_intentions, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ age, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ gender, 
              data = autism_ds, family = "binomial")
...
reduced_model <- glm(highProb ~ 1, 
              data = autism_ds, family = "binomial")
ethnicity_model <- glm(highProb ~ jundice, 
              data = autism_ds, family = "binomial")
...
A drop-in-deviance test between a logistic null model without predictor variables and a logistic model with a single predictor was systematically conducted across all predictor variables as a means of assessing which predictors provide a statistically significant improvemnet in model fit against the null condition. The hypothesis for this test can be observed below: $$H_0: \beta_1 = 0$$ $$H_a: \beta_1 \neq 0$$ - Table 1: Drop-in-deviance test results comparing null model $log(\frac{\hat\pi}{1-\hat\pi})=\beta_0$ to single-predictor model $log(\frac{\pi}{1-\pi})=\beta_0 + \beta_1X_1$
...
tribble(
  ...
  "Q2",	"62.96",	"2.11x10^-15",
  "Q3",	"69.97",	"6.04x10^-17",
  "Q4",	"121.68",	"2.70x10^-28",
  "Q5",	"148.94", "2.95x10^-34",
"Q6",	"217.14",	"3.81x10^-49",
"Q7", "219.73",	"1.03x10^-49",
"Q8",	"77.51",	"1.32x10^-18",
"Q9",	"253.13",	"5.39x10^-57",
"Q10",	"97.29",	"5.99x10^-23",
"Age",	"1.54",	"0.22",
...
From the results of this test, it is evident that the parameters that are statistically significant for model fit are Q1-10, ethnicity, gender, and, quite surprisingly, jaundice when compared to a null model as the p-values are either significantly less than 0.05 (Q1-10 + ethnicity) or slighly below it (gender and jaundice). Due to this consideration, the first model to be analyzed will be a simple additive model that takes into account all these statistically significant predictors.

**Fitting the model** As said before, an additive logistic regression model of the following form was fitted: $$log(\frac{\hat\pi}{1-\hat\pi})=\beta_0 + \beta_1X_{Q1}+...+\beta_{10}X_{Q10}+\beta_{ethnicity}X_{ethnicity}+\beta_{gender}X_{gender}+\beta_{jaundice}X_{jaundice}$$ Given that the condition we established for the "high probability" (i.e. total score greater than 6) allows the logistic model to perfectly separate the response variable in the data into "high" and "low" probabiities (thus making the respective MLE for $\beta_j$ go to infinity), a penalized logistic regression model (LASSO) was employed (Marin, 2021).
...
auc(roc_curve)
```
The area under the curve is 1, this tells us that the model is able to perfectly fit the data throughout all thresholds. This could very well be due to over fitting, however, we lack training data as a means of assesing the model. Potential future direction would be to test this penalized model on another data set.
## Results
